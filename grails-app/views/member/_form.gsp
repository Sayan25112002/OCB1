...
<div class="form-group">
    ...
    <g:textField name="middleName" class="form-control" value="${member?.middleName}" placeholder="Please enter Middle Name"/>
</div>
...
<g:if test="${!(member?.memberType==com.grails.ocb.GlobalConfig.USER_TYPE.ADMINISTRATOR)}">
    <div class="form-group">
        <label><g:message code="course.name"/></label>
        <UIHelper:course value="${member?.courses?.find()?.id}"/>
    </div>
</g:if>