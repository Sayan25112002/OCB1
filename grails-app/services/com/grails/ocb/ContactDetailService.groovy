...
class ContactDetailService {
    ...
    private def saveOrUpdate(def map){
        ...
        if(map&&map.id){
            contactDetails=getById(map.id)?:new ContactDetail()
            contactDetails.properties=map
        }else{
            contactDetails=new ContactDetail(map)
        }
        contactDetails.save(flush:true)
    }

    def createOrUpdate(Contact contact, def params){
    ...
    def deleteContactDetails(Serializable id){
        ContactDetail contactDetail=getById(id)
        if(contactDetail){
            contactDetail.delete(flush:true)
            return AppUtil.infoMessage("Deleted",true)
        }
    ...
    def getContactDetailsListByContact(Contact contact){
        if(contact){
            return ContactDetail.createCriteria().list {
                eq("contact",contact)
            }
        }
        return []
